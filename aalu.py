...
def get_main_keyboard(user_id):
    if user_id == ADMIN_ID:
        keyboard = [
            [KeyboardButton("
 Search Number"), KeyboardButton("
 My Account")],
            [KeyboardButton("
 Credits"),        KeyboardButton("
 Refer")],
            [KeyboardButton("
 Buy Credits"),    KeyboardButton("
 Help")],
            [KeyboardButton("
 Admin Panel")],
        ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 Join Channel", url=FORCE_CHANNEL_LINK)],
            [InlineKeyboardButton("
 Join Group 1", url=FORCE_GROUP1_LINK)],
            [InlineKeyboardButton("
 Join Group 2", url=FORCE_GROUP2_LINK)],
        ]
 ...
 You have been banned from using this bot.")
    ...
    inline_keyboard = [
        [InlineKeyboardButton("
 Channel", url=FORCE_CHANNEL_LINK), InlineKeyboardButton("
 Group 1", url=FORCE_GROUP1_LINK)],
        [InlineKeyboardButton("
 Group 2", url=FORCE_GROUP2_LINK)],
    ]
    unlimited_note = "\n
 Unlimited Mode is currently ON 
 searches are free!\n" if UNLIMITED_MODE else ""
    welcome = f"
 Welcome! You received {START_CREDITS} free credits!\n\n" if is_new else "
 Welcome back!\n\n"
    msg = (
...
async def process_number(update, context, number):
 ...
 Refer button to get your refer link or dm @DarkGalaxxyy to buy credits.")
            return
    try:
...
\n\n"
    for i, u in enumerate(all_users, 1):
        ...
        msg += f"{i}. [{name}](tg://user?id={u['user_id']}) 
 {u['referrals']} refers\n"
    msg += "\n
    await update.message.reply_text(msg, parse_mode="Markdown")
# ============================================================
...
\n\n"
    for i, u in enumerate(top_users):
        ...
        name  = f"@{u['username']}" if u.get("username") else u.get("name") or f"User {u['user_id']}"
        msg  += f"{medal} [{name}](tg://user?id={u['user_id']}) 
 {u['referrals']} refers\n"
    msg += "\n
    await update.message.reply_text(msg, parse_mode="Markdown")
# ============================================================
...
async def createvoucher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 3:
        await update.message.reply_text("
 Usage: /createvoucher <code> <credits> <max_uses>")
        return
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 Join Group 2", url=FORCE_GROUP2_LINK)],
        ...
        await update.message.reply_text("
 Please join all required channels/groups before redeeming.", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    if not context.args:
        await update.message.reply_text("
 Usage: /redeem <code>")
...
async def deletevoucher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("
 Usage: /deletevoucher <code>")
        return
    code   = context.args[0].upper()
    result = await vouchers.delete_one({"code": code})
    await update.message.reply_text(f"
 Voucher '{code}' deleted." if result.deleted_count else f"
...
async def buy_credits_menu(update, context):
 ...
 Pay via UPI", callback_data="buy_upi")]]
    await update.message.reply_text(
        "
 Buy Credits\n\n
 Rate: 
...
\n\n"
 ...
 Order ID: {order_id}"
        ...
        status_keyboard = [[InlineKeyboardButton("
 Mark as Done", callback_data=f"done_{order_id}_{user_id}_{amount}")]]
 ...
 UPI amount entry 
    if context.user_data.get("upi_step") == "enter_amount":
        if text.isdigit() and int(text) > 0:
            ...
            keyboard = [
                [InlineKeyboardButton("
 I've Paid", callback_data=f"paid_{order_id}")],
                [InlineKeyboardButton("
 Cancel",    callback_data="cancel_payment")]
            ]
...
{amount}\n"
                    f"
 Credits you'll receive: {amount}\n\n"
                    f"
 UPI ID: `{UPI_ID}`\n\n"
                    f"Pay and click 
 I've Paid button below."
                ),
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
 ...
 Enter without +91\n"
    ...
    if context.user_data.get("waiting_for_number"):
        ...
        if text.isdigit():
            ...
            await update.message.reply_text("
 Invalid number. Please enter digits only, without +91 or spaces.")
        return
    # 
 My Account 
    if text == "
 My Account":
        ...
        unlimited_note = "\n
 Unlimited Mode ON 
 searches are free!" if UNLIMITED_MODE else ""
        await update.message.reply_text(
            "
 My Account\n
\n\n"
 ...
 User ID: {user_id}\n"
            f"
 Credits: {user['credits']}{unlimited_note}\n"
            f"
 Joined: {user['joined']}\n"
            f"
 Referrals: {user['referrals']}\n"
...
\n\n"
 ...
 How to get Credits:\n\n"
            f"
 New users get {START_CREDITS} free credits\n"
            f"
 Refer a friend 
 earn {REFER_CREDITS} credits\n"
            "
 Redeem a voucher 
 /redeem <code>\n"
            "
...
\n\n"
            ...
            f"
 Your total referrals: {user['referrals']}\n"
            "
        )
        return
    # 
...
\n\n"
 ...
 Mode: {MODE}\n"
            f"
 Unlimited: {'ON' if UNLIMITED_MODE else 'OFF'}\n"
            f"
 Total Users: {total_users}\n"
            f"
 Total Credits: {total_credits}\n"
            f"
 Start Credits: {START_CREDITS}\n"
            f"
 Refer Credits: {REFER_CREDITS}\n"
            f"
...
async def setmode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].lower() not in ["dual", "group", "private", "maintenance"]:
        await update.message.reply_text("
 Usage: /setmode dual|group|private|maintenance")
        return
...
async def unlimited(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 Usage: /unlimited on|off")
    ...
    if val == "on":
        UNLIMITED_MODE = True
        await update.message.reply_text("
 Unlimited Mode is now ON")
    elif val == "off":
        UNLIMITED_MODE = False
        await update.message.reply_text("
 Unlimited Mode is now OFF")
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    # Get exact message as typed 
 no format change
...
async def addcredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 {amount} credits have been added to your account.\n"
                f"
 Current Balance: {new_bal} credits\n\n"
...
async def removecredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 Usage: /removecredits <user_id> <amount>")
    ...
    await update.message.reply_text(f"
 Removed {context.args[1]} credits\n
 New Balance: {new_bal}" if new_bal is not None else "
 User not found.")
async def setcredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 Usage: /setcredits <user_id> <amount>")
    ...
    await update.message.reply_text(f"
 Credits set to {context.args[1]}" if success else "
 User not found.")
...
async def setstartcredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("
 Usage: /setstartcredits <amount>")
        return
    START_CREDITS = int(context.args[0])
    await update.message.reply_text(f"
 Start credits set to: {START_CREDITS}")
async def setrefercredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("
 Usage: /setrefercredits <amount>")
        return
    REFER_CREDITS = int(context.args[0])
    await update.message.reply_text(f"
 Refer credits set to: {REFER_CREDITS}")
# ============================================================
#                     MAIN
# ============================================================
